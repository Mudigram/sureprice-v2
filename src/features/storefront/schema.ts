...
export const updateStorefrontStudioSchema = z.object({
  ...
  description: z.string().max(1000, 'Story / Description must be 1000 characters or less').nullable().optional(),
  ...
  stall_number: z.string().max(50, 'Stall number too long').nullable().optional(),
  location_zone: z.string().max(100, 'Zone name too long').nullable().optional(),
  directions_landmark: z.string().max(200, 'Landmark too long').nullable().optional(),
  bank_enabled: z.boolean().default(false),
  bank_name: z.string().max(60, 'Bank name too long').nullable().optional(),
  account_number: z.string().max(30, 'Account number too long').nullable().optional(),
  account_name: z.string().max(100, 'Account name too long').nullable().optional(),
  social_instagram: z.string().max(100, 'Instagram handle too long').nullable().optional(),
  social_tiktok: z.string().max(100, 'TikTok handle too long').nullable().optional(),
  social_twitter: z.string().max(100, 'Twitter handle too long').nullable().optional(),
  social_facebook: z.string().max(100, 'Facebook page too long').nullable().optional(),
  social_website: z.string().max(150, 'Website URL too long').nullable().optional(),
  payments: z.array(z.string()).default(['Cash', 'Bank Transfer', 'POS Card']),
